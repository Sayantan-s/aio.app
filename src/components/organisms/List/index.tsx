...
export const List = <TData,>({
    ...
    return (
        <div className={`dark:bg-slate-900/50 rounded-lg flex-1 overflow-hidden flex flex-col ${className || ''}`}>
            <div className="relative flex flex-col h-full">
                <div className="flex-1 absolute top-0 left-0 right-0 bottom-0 overflow-hidden flex flex-col">
                    <div className="flex flex-col space-y-3 overflow-y-auto scrollbar-hide news pb-4" onScroll={onHandleScroll}>
                        {renderHeader ? (
                            <header className={`sticky top-0 w-full z-20 shadow-xl`}>{renderHeader(hasScrolledFromTop)}</header>
                        ) : null}
